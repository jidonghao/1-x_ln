...
typedef struct
{//以下所有时间单位：MS
	uint8_t numBlinksGlobal[HAL_BUZZER_DEFAULT_MAX_BUZZERS];//闪烁次数
	uint16_t percentGlobal[HAL_BUZZER_DEFAULT_MAX_BUZZERS];//一个周期中亮的时长
	uint16_t periodGlobal[HAL_BUZZER_DEFAULT_MAX_BUZZERS];//一个周期时长
	uint16_t BuzzerTimeCount[HAL_BUZZER_DEFAULT_MAX_BUZZERS];//每个周期中，已经走过的时间计时
} BuzzerState_TypeDef;
