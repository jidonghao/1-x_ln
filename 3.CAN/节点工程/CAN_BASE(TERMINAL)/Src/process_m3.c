...
void Master_To_Gateway(uint8_t StdId,ValueType Value_Type,uint16_t vol,uint8_t  switching,uint16_t sensor_hum,uint16_t sensor_tem ) //CAN节点网关上发协议到网关 以便于在云平台展示
{

	memset(MASTER_TX_BUF,0x00,USART2_DATA_FIFO_BUF_SIZE);
	MASTER_TX_BUF[0]=START;
	MASTER_TX_BUF[1]=StdId;
	MASTER_TX_BUF[2]=CMD;
	MASTER_TX_BUF[3]=LEN;
	MASTER_TX_BUF[4]=Sensor_Type_t;
	switch(Value_Type)
	{
		case Value_ADC:						
					MASTER_TX_BUF[5] = ((vol&0xff00)>>8);				
					MASTER_TX_BUF[6] = vol&0x00ff;			
			break;
		case Value_Switch:
					MASTER_TX_BUF[5] = 0x00;
					MASTER_TX_BUF[6] = switching;
			break;
		case Value_I2C:
			    MASTER_TX_BUF[5]=sensor_tem;
					MASTER_TX_BUF[6]=sensor_hum;
//			sprintf((char *)(DEVICE_TX_BUF+6),"%c%d%c%c%c%d",'T',sensor_tem,',','R','H',sensor_hum);
			break;
		default:
			break;
	}
	MASTER_TX_BUF[7]=CHK(MASTER_TX_BUF,7);
	MAX3485_ConTx() ; 
	USART2_SendArr(MASTER_TX_BUF,LEN+1);
	MAX3485_ConRx();	
	printf("发送本机传感器数据至网关 MASTER_TX_BUF = %02X %02X %02X %02X %02X %02X %02X %02X \r\n",MASTER_TX_BUF[0],MASTER_TX_BUF[1],MASTER_TX_BUF[2],MASTER_TX_BUF[3],MASTER_TX_BUF[4],MASTER_TX_BUF[5],MASTER_TX_BUF[6],MASTER_TX_BUF[7]);
...
void CAN_Master_To_Gateway( uint8_t *can_data,uint8_t len)
{
	memset(MASTER_TX_BUF,0x00,USART2_DATA_FIFO_BUF_SIZE);
	MASTER_TX_BUF[0]=START;
	MASTER_TX_BUF[1]=can_data[4];
	MASTER_TX_BUF[2]=CMD;
	MASTER_TX_BUF[3]=LEN;
	MASTER_TX_BUF[4]=can_data[0];
	MASTER_TX_BUF[5]=can_data[1];
	MASTER_TX_BUF[6]=can_data[2];
//sprintf((char *)(DEVICE_TX_BUF+6),"%c%d%c%c%c%d",'T',sensor_tem,',','R','H',sensor_hum);
	MASTER_TX_BUF[7]=CHK(MASTER_TX_BUF,7);
	...
	USART2_SendArr(MASTER_TX_BUF,LEN+1);
	MAX3485_ConRx();	
	printf("发送CAN节点数据至网关 MASTER_TX_BUF = %02X %02X %02X %02X %02X %02X %02X %02X \r\n\r\n",MASTER_TX_BUF[0],MASTER_TX_BUF[1],MASTER_TX_BUF[2],MASTER_TX_BUF[3],MASTER_TX_BUF[4],MASTER_TX_BUF[5],MASTER_TX_BUF[6],MASTER_TX_BUF[7]);
