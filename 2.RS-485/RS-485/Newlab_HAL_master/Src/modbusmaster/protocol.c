...
m_send_frame_typedef   m_send_frame;
m_rev_frame_typedef    m_rev_frame;
...
void clr485rxbuf(void)
{
    ...
    mymemset(&m_send_frame, 0x00, sizeof(m_send_frame_typedef)); //清空缓存
    mymemset(&m_rev_frame, 0x00, sizeof(m_send_frame_typedef)); //清空缓存
    m_ctrl_dev.rxlen = 0;
...
//打包一帧数据，并发送
//fx：指向需要打包的帧
void mb_packsend_frame(m_rev_frame_typedef *fx)
{
    u16 i;
    u16 calchkval=0;			//计算得到的校验值
