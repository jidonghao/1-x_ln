...
void usmart_reset_runtime(void)
{
    ...
    __HAL_TIM_SET_AUTORELOAD(&htim2,0XFFFF);     //将重装载值设置到最大
    __HAL_TIM_SET_COUNTER(&htim2,0);             //清空定时器的CNT
...
{
    if(__HAL_TIM_GET_FLAG(&htim2,TIM_FLAG_UPDATE)==SET) { //在运行期间,产生了定时器溢出
...
{
    if(__HAL_TIM_GET_IT_SOURCE(&htim2,TIM_IT_UPDATE)==SET) { //溢出中断
     //    DBG_B_INFO("test TIM2 interupt");
        usmart_dev.scan();	//执行usmart扫描
        __HAL_TIM_SET_COUNTER(&htim2,0);;    //清空定时器的CNT
        __HAL_TIM_SET_AUTORELOAD(&htim2,100);//恢复原来的设置
    }
...
//    HAL_NVIC_SetPriority(TIM2_IRQn,3,3);    //设置中断优先级，抢占优先级3，子优先级3
...
//    HAL_TIM_Base_Start_IT(&TIM4_Handler); //使能定时器4和定时器4中断
    ...
    HAL_TIM_Base_Start_IT(&htim2); //使能定时器4和定时器4中断
}
