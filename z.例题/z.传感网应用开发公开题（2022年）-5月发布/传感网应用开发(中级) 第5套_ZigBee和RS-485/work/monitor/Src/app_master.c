...
void master_get_slave(void)//ADD class_sen[2] CJW
{
    class_sen[0].add=1;
		class_sen[0].senty=Photosensitive_Sensor; 		
    class_sen[1].add=2;
    class_sen[1].senty=FlammableGas_Sensor;		
		class_sen[2].add=3;
		class_sen[2].senty=TemHum_Sensor; 
    master_init=1;
}
...
static void mater_push(u8 i)
{
	/*开始完善代码——按照考试资源目录\通信协议目录里提供的《RS-485网
	络上报物联网网关通讯协议》将485从机发来的传感器数据发送至网关
	注：此函数传参i为从机发来传感器数据组（类型）的遍历序号  */
	
    u8 push485buf[9];

    push485buf[0]=0xDD;	 
    push485buf[1]=class_sen[i].add;
		push485buf[2]=0x00;
    push485buf[3]=0x02;
    push485buf[4]=0x09;

    switch (class_sen[i].senty) {
    ...
    push485buf[6]=(u8)(class_sen[i].value>>8);
    push485buf[7]=(u8)class_sen[i].value;
