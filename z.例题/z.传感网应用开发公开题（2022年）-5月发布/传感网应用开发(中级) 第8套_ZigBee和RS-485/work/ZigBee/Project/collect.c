...
static uint8 pTxData[MAX_SEND_BUF_LEN]; //定义无线发送缓冲区的大??static uint8 pRxData[MAX_RECV_BUF_LEN]; //定义无线接收缓冲区的大小
/**************************************************/
static basicRfCfg_t basicRfConfig;


//使LED n 闪烁 time 毫秒
#define FlashLed(n, time) \
  do                     \
    {                      \
      halLedSet(n);       \
        halMcuWaitMs(time); \
...
void ConfigRf_Init(void)
{
  ...
  basicRfConfig.panId = PAN_ID;       // zigbee的ID号设??   basicRfConfig.channel = RF_CHANNEL; // zigbee的频道设??   basicRfConfig.myAddr = MY_ADDR;     //设置本机地址
  basicRfConfig.ackRequest = TRUE;    //应答信号
  while (basicRfInit(&basicRfConfig) == FAILED);                //检测zigbee的参数是否配置成??   basicRfReceiveOn(); // 打开RF
...
*输入：uint8 *input-指向输入缓存?? uint8 len-输入数据的字节长?? *输出：uint8 *output-指向输出缓存?? *返回：返回生成字符串的长?? *特殊说明：无
******************************************************************************************/
uint8 GetHexStr(uint8 *input, uint8 len, uint8 *output)
{
  ...
  {
    sprintf(str + i * 3,"%02X ",*input); //注意，字符串间以空格隔开??%02X "双引号中有一个空??      input++;
  }
  ...
  while (1)
  {
    ...
    if (basicRfPacketIsReady()) //查询有没收到无线信号
    {
      ...
      if(rxLen >= 8)
      {
        ...
        /* 开始完善代码——收到数据后，计数值增??，并更新计数D3 D6 D5指示灯的显示 */
        setLedWhile(rx_cnt++);
        /* 结束完善代码——收到数据后，计数值增??，并更新计数D3 D6 D5指示灯的显示 */
      }
    }
  }
}